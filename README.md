# Whisper base.en FP32 vs Parakeet-TDT v2 INT8 + Hotwords — v10.4 WORKERFS
...
```yaml
key: parakeet-wasm-${{ env.SHERPA_VERSION }}-workerfs-v10.4
```
...
### v10.3
Le workflow n'ajoute plus `WORKERFS` à `EXPORTED_RUNTIME_METHODS`. Il exporte seulement `FS` et lie `-lworkerfs.js`; le backend est récupéré via `FS.filesystems.WORKERFS`. La clé de cache runtime passe à `workerfs-v10.4` pour forcer un rebuild propre.
